"""KBQuant 压力测试 — 用 QuantClient 逐步递增并发，找到服务极限。

每个阶段并发翻倍，记录成功率、延迟分布、连接错误等。
"""
...
CLIENT_INSTANCES = 8  # 每个阶段用固定数量的客户端实例（复用连接池）


async def one_request(client: QuantClient) -> dict:
    """单次请求 — 调用 information.list，一次返回 10 条。"""
    t0 = time.perf_counter()
    try:
        result = await client.information.list(page_size=10)
        elapsed = time.perf_counter() - t0
        return {"ok": True, "elapsed": elapsed, "total": result.total}
...
async def run_phase(concurrency: int, num_clients: int) -> dict:
    """用 num_clients 个 QuantClient 实例，跑 concurrency 个并发请求。"""
    clients = [
    ...
    t0 = time.perf_counter()
    tasks = [one_request(clients[i % num_clients]) for i in range(concurrency)]
    ...
    p50 = latencies[int(total_ok * 0.5)] if total_ok > 0 else 0
    p95 = latencies[int(total_ok * 0.95)] if total_ok > 0 else 0
    p99 = latencies[int(total_ok * 0.99)] if total_ok > 0 else 0
    max_lat = latencies[-1] if total_ok > 0 else 0
    min_lat = latencies[0] if total_ok > 0 else 0
...
async def main():
    print("=" * 90)
    print(f"KBQuant 压力测试 — 目标: {BASE_URL}")
    print(f"客户端实例数: {CLIENT_INSTANCES}，每个 client max_connections=500")
    print("每轮请求: client.information.list(page_size=10)")
